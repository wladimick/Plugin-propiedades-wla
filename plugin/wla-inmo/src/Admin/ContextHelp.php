...
final class ContextHelp
{
	public static function add($screen): void
	{
		...
		if ($isProperty) {
			...
			$content .= '<h3>' . esc_html__('CSV o JSON WLA', 'wla-inmo') . '</h3>';
			$content .= '<p>' . esc_html__('CSV permite mapear columnas manualmente. JSON WLA ya contiene un contrato versionado: su origen y sus campos se validan en servidor y no se remapean desde el navegador.', 'wla-inmo') . '</p>';
			$content .= '<h3>' . esc_html__('Simular antes de confirmar', 'wla-inmo') . '</h3>';
			$content .= '<p>' . esc_html__('Ambos formatos pasan por dry-run antes de crear o modificar propiedades. Un dry-run con errores no se puede confirmar.', 'wla-inmo') . '</p>';
			$content .= '<h3>' . esc_html__('Reanudar un batch', 'wla-inmo') . '</h3>';
			$content .= '<p>' . esc_html__('Cada lote avanza desde un checkpoint. Si se pausa o falla, usa Continuar/Reanudar; no vuelvas a subir el archivo para intentar continuar el mismo batch.', 'wla-inmo') . '</p>';
			$content .= '<h3>' . esc_html__('Respaldo JSON', 'wla-inmo') . '</h3>';
			$content .= '<p>' . esc_html__('El respaldo JSON WLA exporta campos portables por páginas y excluye por defecto IDs externos, direcciones privadas, notas internas y otros campos marcados como privados.', 'wla-inmo') . '</p>';
