...
final class ValueNormalizer
{
	...
	public static function normalize(mixed $rawValue, array $definition, ?string $separator = null): NormalizedValue
	{
		...
		return match ($validator) {
			...
			'url' => self::url($rawValue),
			'url_list' => self::urlList($rawValue, $separator, (int) ($definition['max_items'] ?? 0)),
	...
	{
		if (is_bool($value)) { return NormalizedValue::valid($value); }
		if (is_int($value) || is_float($value)) {
			if ((float) $value === 1.0) { return NormalizedValue::valid(true); }
			if ((float) $value === 0.0) { return NormalizedValue::valid(false); }
			return NormalizedValue::invalid('invalid_boolean');
		}
		if (!is_string($value)) { return NormalizedValue::invalid('invalid_boolean'); }
		$normalized = strtolower(trim($value));
		if (in_array($normalized, array('1', 'true', 'yes', 'on', 'si', 'sí'), true)) { return NormalizedValue::valid(true); }
		if (in_array($normalized, array('0', 'false', 'no', 'off'), true)) { return NormalizedValue::valid(false); }
		return NormalizedValue::invalid('invalid_boolean');
	}

	private static function integer(mixed $value): NormalizedValue
	{
		if ($value === '' || $value === null || is_bool($value) || filter_var($value, FILTER_VALIDATE_INT) === false) { return NormalizedValue::invalid('invalid_integer'); }
	...
	private static function status(mixed $value): NormalizedValue
	{
		...
		return in_array($status, self::ALLOWED_STATUSES, true) ? NormalizedValue::valid($status) : NormalizedValue::invalid('invalid_status');
	}
	...
	private static function url(mixed $value): NormalizedValue
	{
		$url = Sanitizer::httpUrl($value);
		return $url === '' ? NormalizedValue::invalid('invalid_url') : NormalizedValue::valid($url);
	}

	private static function urlList(mixed $value, ?string $separator, int $maxItems = 0): NormalizedValue
	{
		$items = self::splitValues($value, $separator);
		if ($maxItems > 0 && count($items) > $maxItems) {
			return NormalizedValue::invalid('too_many_values');
		}
		$urls = array();
		foreach ($items as $item) {
			$url = Sanitizer::httpUrl($item);
			if ($url === '') { return NormalizedValue::invalid('invalid_url'); }
			$urls[] = $url;
		}
		$urls = array_values(array_unique($urls));
		if ($maxItems > 0 && count($urls) > $maxItems) {
			return NormalizedValue::invalid('too_many_values');
		}
		return NormalizedValue::valid($urls);
	}
	...
	private static function taxonomyValue(mixed $value, ?string $separator, bool $multiple): NormalizedValue
	{
		...
		if ($items === array()) { return NormalizedValue::invalid('empty_taxonomy_value'); }
		if (!$multiple && count($items) > 1) { return NormalizedValue::invalid('multiple_values_for_single_target'); }
	...
	{
		if (is_array($value)) { return array_map(static fn (mixed $item): string => trim((string) $item), $value); }
		if (!is_scalar($value) && $value !== null) { return array(); }
		$string = trim((string) $value);
		if ($string === '') { return array(); }
		if ($separator === null || $separator === '') { return array($string); }
