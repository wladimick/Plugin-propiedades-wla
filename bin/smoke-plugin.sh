...
required_files=(
	...
	"src/Settings/RewriteManager.php"
...
if grep -RIEq 'flush_rewrite_rules[[:space:]]*\(' "$PLUGIN_DIR/src/Admin/SettingsPage.php" "$PLUGIN_DIR/src/Settings/Schema.php" "$PLUGIN_DIR/src/Settings/Registry.php"; then
	echo "Settings save/sanitization must never flush rewrite rules directly." >&2
	exit 1
fi

if [[ "$(grep -c 'flush_rewrite_rules(false)' "$PLUGIN_DIR/src/Settings/RewriteManager.php")" -ne 1 ]]; then
	echo "Controlled settings rewrite manager must contain exactly one hard rewrite flush." >&2
	exit 1
fi
...
if grep -RIEq 'private_address|internal_notes|external_id|business_email|business_phone|whatsapp_number' "$PLUGIN_DIR/src/Dashboard"; then
	echo "Dashboard snapshot must not read private property or contact fields." >&2
	exit 1
fi

if grep -RIEq 'Search\\Index|wp_remote_|curl_|XMLHttpRequest|axios|\.ajax[[:space:]]*\(' "$PLUGIN_DIR/src/Dashboard" "$PLUGIN_DIR/src/Admin/DashboardPage.php"; then
	echo "Dashboard must remain administrative, local and independent from the published-only search index." >&2
	exit 1
fi
